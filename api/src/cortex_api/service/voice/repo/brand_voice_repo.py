# api/src/cortex_api/service/voice/repo/brand_voice_repo.py
...
_IN_FLIGHT = (VoiceJobStatus.PENDING, VoiceJobStatus.RUNNING)


class BrandVoiceRepo:
    """Brand-scoped access to the `brand_voice` table."""

    async def get(self, session: AsyncSession, brand_id: UUID) -> BrandVoice | None:
        result = await session.exec(select(BrandVoice).where(BrandVoice.brand_id == brand_id))
        return result.first()

    async def find_in_flight(self, session: AsyncSession, brand_id: UUID) -> BrandVoice | None:
        row = await self.get(session, brand_id)
        return row if row and row.status in _IN_FLIGHT else None

    async def create(self, session: AsyncSession, job: BrandVoice) -> BrandVoice:
        ...
        with `brand_profile.updated_at` — `VoiceJobService.start` compares the
        two across tables, so they must not come from different clocks.
        """
        stmt = (
            pg_insert(BrandVoice)
            .values(
            ...
            .returning(BrandVoice)
        )
        result = await session.execute(stmt)
        saved: BrandVoice = result.scalars().one()
        await session.flush()
        return saved
    ...
    async def mark_running(self, session: AsyncSession, brand_id: UUID) -> None:
        ...
        if job is None:
            raise ValueError(f"BrandVoice not found for brand_id={brand_id}")
        job.status = VoiceJobStatus.RUNNING
    ...
    async def mark_succeeded(self, session: AsyncSession, brand_id: UUID, samples: dict[str, str]) -> None:
        job = await self.get(session, brand_id)
        if job is None:
            raise ValueError(f"BrandVoice not found for brand_id={brand_id}")
        job.status = VoiceJobStatus.SUCCEEDED
        job.samples = samples
        job.error = None
        job.updated_at = sa.func.now()  # type: ignore[assignment]  # DB clock — see create()
        session.add(job)
    ...
    async def sweep_stale(self, session: AsyncSession, older_than_seconds: int) -> int:
        ...
        result = await session.exec(
            select(BrandVoice).where(
                BrandVoice.status.in_([s.value for s in _IN_FLIGHT]),  # type: ignore[attr-defined]
                BrandVoice.updated_at < cutoff,
            )
        )
