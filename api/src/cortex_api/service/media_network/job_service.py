...
class MediaNetworkJobService:
    """Owns the media-network job lifecycle and the in-process ranking worker.

    Structural mirror of AnalyzeJobService: ``_tasks`` dict, create_task +
    ...
    Key deviation from AnalyzeJobService: BrandMediaNetwork uses brand_id as
    the sole PK (no separate job_id row), so ``_tasks`` maps each task to a
    ...
    def __init__(
        ...
        # AnalyzeJobService._tasks — the add_done_callback below pops on
    ...
    async def start(self, brand_id: UUID, regenerate: bool = False) -> BrandMediaNetwork:
        """Start (or return) a media-network job for brand_id.

        Dedupe logic mirrors AnalyzeJobService.start_analyze:
        ...
        - A SUCCEEDED row generated BEFORE the brand profile was last updated is
          stale: the profile is the matcher's input, so re-extracting it
          ...
          media network. Self-heals on the next start() with no caller change.
        ...
        than event-driven, matching the weekly-questions and voice services.
        All three are profile-derived caches that each rediscover staleness
        independently; a shared invalidation seam (or a `BrandProfileUpdated`
        domain event) is the right consolidation now that three caches need
        this signal — tracked as a follow-up rather than blocking this fix.
        """
        async with self._db.session() as session:
            existing = await self._jobs.get(session, brand_id)
            stale_vs_profile = False
            profile_updated_at = None
            if existing is not None and existing.status == MediaJobStatus.SUCCEEDED:
            ...
            existing_is_dedupable = existing is not None and (
                existing.status in (MediaJobStatus.PENDING, MediaJobStatus.RUNNING)
                or (existing.status == MediaJobStatus.SUCCEEDED and not stale_vs_profile)
            )
            ...
            # "media network shows the previous brand" bug otherwise requires
            # reading both updated_at columns out of the DB by hand.
            self._logger.info(
                "media_job_regenerate",
                brand_id=str(brand_id),
            ...
            job = await self._jobs.create(session, BrandMediaNetwork(brand_id=brand_id))
    ...
    def _regenerate_reason(
        regenerate: bool,
        existing: BrandMediaNetwork | None,
        ...
        if existing is None:
            return "no_row"
        if existing.status == MediaJobStatus.FAILED:
            return "prior_failed"
        if stale_vs_profile:
    ...
    async def cancel_all(self) -> None:
        ...
        Mirrors AnalyzeJobService.cancel_all. Snapshots the in-flight set
        ...
        for task, brand_id in in_flight:
            try:
                ...
                self._logger.exception(
                    "media_cancel_fail_write_failed",
                    brand_id=str(brand_id),
                )
            task.cancel()
        await self.drain()
    ...
    async def _run(self, brand_id: UUID) -> None:
        ...
        Session split is deliberate (mirrors AnalyzeJobService._run):
        - mark_running commits in its OWN session so RUNNING is visible to
          dedupe and stale-sweep while work is in flight.
        - mark_succeeded commits in ONE session with the outlets payload so
          the write is atomic — never a half-state of outlets-written-but-status-RUNNING.
        """
        try:
            async with self._db.session() as session:
                await self._jobs.mark_running(session, brand_id)
                profile_row = await self._brand_profile_repo.get(session, brand_id)
                catalog: Sequence[MediaNetworkMember] = await self._members.list_all(session)
            profile = {
            ...
            async with self._db.session() as session:
                await self._jobs.mark_succeeded(session, brand_id, outlets)
    ...
    async def _fail(self, brand_id: UUID, message: str) -> None:
        """Best-effort mark-failed helper — mirrors AnalyzeJobService._fail."""
        try:
            async with self._db.session() as session:
            ...
            self._logger.exception("media_fail_write_failed", brand_id=str(brand_id))
            return
        self._logger.warning("media_job_failed_marked", brand_id=str(brand_id))
