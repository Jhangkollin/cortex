...
/**
 * Server Actions — weekly-questions pipeline bridge for the onboarding v2 wizard.
 *
 ...
 * actions wrap SP-QUESTIONS's `startWeeklyQuestions` / `pollWeeklyQuestions`,
 * deriving `brandId` + signed-token claims from the authenticated NextAuth
 * session server-side so nothing scoping-related is ever trusted from the
 * client.
 *
...
import { type WeeklyQuestionsDTO, pollWeeklyQuestions, startWeeklyQuestions } from "@/lib/cortex-api";
...
export async function startWeeklyQuestionsAction(): Promise<WeeklyQuestionsDTO> {
  const { claims, brandId } = await claimsFromSession();
  return startWeeklyQuestions(claims, brandId);
}

export async function pollWeeklyQuestionsAction(): Promise<WeeklyQuestionsDTO> {
  const { claims, brandId } = await claimsFromSession();
  return pollWeeklyQuestions(claims, brandId);
}
