/**
 * Demo-mode behaviour for the shared zh wizard. Pairs with wizard.live.test.tsx;
...
describe("OnboardingV2WizardZh — mode='demo'", () => {
  beforeEach(() => {
  ...
  it("renders step 0 on mount even if the live completion flag is pre-set", async () => {
    ...
    render(<OnboardingV2WizardZh mode="demo" api={makeOkApi()} />);
  ...
  it("稍後再設定 does NOT write to localStorage in demo mode", () => {
    render(<OnboardingV2WizardZh mode="demo" api={makeOkApi()} />);
  ...
  it("進入 Discover restarts the wizard and never calls router.push", async () => {
    render(<OnboardingV2WizardZh mode="demo" api={makeOkApi()} />);
