 ...
 * "<Brand> 加入了你的品牌組合", renders a PortfolioBand, and adds a
 * secondary "新增品牌" CTA card alongside the primary "進入 Discover".
 */
...
const LIVE_AGENTS: Array<{ name: string; icon: string; status: string; accent: "amber" | "teal" }> = [
  { name: "Answer Pilot", icon: "edit_note", status: "草擬 3 篇", accent: "amber" },
  { name: "GEO Pilot", icon: "explore", status: "校準分發", accent: "teal" },
  { name: "Market Radar", icon: "radar", status: "掃描競品", accent: "teal" },
  { name: "Monetize Lens", icon: "insights", status: "歸因追蹤", accent: "teal" },
  { name: "Brand Voice", icon: "campaign", status: "校準完成", accent: "teal" },
];
...
function SecondaryAddBrandCta({ onClick, busy }: { onClick: () => void; busy?: boolean }) {
  return (
    <div className="rounded-md border border-ink-200 bg-white p-5">
      ...
      <div className="text-xs font-bold uppercase tracking-wide text-ink-500">或者，繼續前進</div>
      <div className="mt-1 text-xl font-bold text-ink-900">再導入一個品牌</div>
      <div className="mt-1 text-sm text-ink-600">
        並排管理子品牌、聯盟品牌或客戶帳號。每個品牌都有自己的 Agent 團隊。
      </div>
      <button
        type="button"
        onClick={onClick}
  ...
  return (
    <div style={{ paddingBottom: 80 }}>
      <div style={{ textAlign: "center", marginBottom: 30 }}>
        ...
        >
          {isMulti
            ? <>{brand.name} 加入了你的品牌組合</>
            : <>{brand.name} 的 Agent 開始工作了</>}
        </h2>
        <p
          style={{
        ...
        >
          {isMulti ? (
            <>
              你現在從同一個 Cortex 工作區管理 <strong style={{ color: "var(--mly-ink-900)" }}>{brands.length} 個品牌</strong>，
              隨時可以從側邊欄切換。
            </>
          ) : (
            <>
              首批 Brand Answer 預估{" "}
              <strong style={{ color: "var(--cortex-amber-600)" }}>23 小時 47 分</strong> 後上線。
              現在你可以進入 Discover 看 Cortex 替你做了什麼。
            </>
          )}
        </p>
      </div>
      ...
      <div className={isMulti ? "grid grid-cols-1 gap-4 lg:grid-cols-2" : undefined}>
        {/* Light Edition (handoff §3.3): cream-paper gradient + paper-border
            + gold-soft icon backplate + gold solid CTA. "重新做一次" drops
            from onDark to the soft variant. */}
        <Card
          style={{
            padding: 22,
            background:
        ...
        >
          ...
          <div>
            ...
            >
              進入 Discover 看 Agent 的進度
            </div>
            <div style={{ fontSize: 13, color: "var(--paper-ink-2)", lineHeight: 1.55 }}>
              Cortex 會在那裡每天主動告訴你：今天有什麼新問題、Agent 在做什麼、要優先處理什麼。
            </div>
          </div>
