...
describe("StepComplete — multi-brand variant", () => {
  it("collapses to single-brand hero when brand_count === 1", () => {
    ...
    // Single-brand fallback: the existing hero text is present
    expect(screen.getByText(/Agent is on the job/i)).toBeInTheDocument();
    // No portfolio band heading
    expect(screen.queryByRole("heading", { name: /your portfolio/i })).not.toBeInTheDocument();
    // No "Add a brand" secondary CTA card button
  ...
  it("shows portfolio band + 'Add a brand' CTA when brand_count >= 2", () => {
    ...
    // Click the secondary CTA card (NOT the band's tile button)
