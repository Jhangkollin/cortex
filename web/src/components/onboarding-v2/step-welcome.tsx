...
const EXTRACTION_LABELS: Array<{ x: string; y: string; label: string; icon: string }> = [
  { x: "20%", y: "22%", label: "Brand name", icon: "badge" },
  { x: "76%", y: "20%", label: "Logo", icon: "interests" },
  { x: "84%", y: "52%", label: "Products × 12", icon: "category" },
  { x: "72%", y: "82%", label: "Brand voice", icon: "campaign" },
  { x: "20%", y: "82%", label: "Competitors × 4", icon: "groups" },
  { x: "12%", y: "52%", label: "Category", icon: "psychology" },
];

const TRUST_STRIP = [
  { i: "shield", l: "Your data stays in your workspace", s: "Internal to Cortex, never sold" },
  // Light Edition (handoff Appendix D): 90s → 60s to match the StepCrawl
  // receipt's "用時 74s" timing — keeps the headline trust-strip promise
  // honest end-to-end.
  { i: "schedule", l: "Crawl runs in ~60s", s: "Edit while it runs" },
  { i: "fact_check", l: "Everything is editable", s: "Cortex just writes the first draft" },
];
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        >
          Point Cortex at your website. We&apos;ll pull your brand name, products, voice and
          competitors — and have an editable draft ready in two minutes. All you have to
          do is review and confirm.
        </p>
        ...
        >
          <span style={{ fontSize: 12, color: "var(--mly-ink-500)" }}>Try:</span>
      ...
      {/* RIGHT — radar visualisation of what we'll extract (Light Edition).
          The deep-teal canvas is replaced with a cream radial. The radar
          stage is a fixed 480×480 square so ring widths/heights cannot
          drift into ellipses when the parent's aspect ratio changes
          (handoff Appendix B). The gold sweep is wrapped in an
          overflow:hidden circle to hard-crop the conic gradient inside
          the outermost ring. */}
      <div style={{ position: "relative", height: 560 }}>
        ...
        >
          {/* Fixed-size square radar stage. The rings, sweep and centre
              disc are all relative to this 480×480 box (not the parent),
              which keeps everything circular regardless of the outer
              column ratio. */}
          <div
            style={{
              position: "absolute",
          ...
          >
            ...
            {/* Gold radar sweep — wrapped in an overflow:hidden circular
                clip the exact size of the outermost ring so the conic
                gradient can never bleed past the dashed edge. */}
            <div
              aria-hidden
              style={{
                position: "absolute",
            ...
            {/* Centre URL globe — brand-teal disc, white earth icon,
                gold dashed ring on the outside. */}
            <div
              style={{
                position: "absolute",
                left: "50%",
          ...
          {/* Extraction pill labels — white cards with paper-border + a
              brand-teal icon. Positioned against the outer rect so they
              spread across the full canvas, not just the 480×480 stage. */}
          ...
          >
            ...
            <div style={{ marginTop: 2 }}>brands onboarded</div>
          </div>
        </div>
      </div>
    </div>
