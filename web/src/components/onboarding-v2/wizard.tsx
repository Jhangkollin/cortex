...
import { EXTRACTED_BRAND, RAIL_STEPS, type InternalStep, railFor } from "@/components/onboarding-v2/data";
...
import { StepPreparingReport } from "@/components/onboarding-v2/step-preparing-report";
...
export function OnboardingV2Wizard({
  ...
  // Used to show the PortfolioBand and "Add a brand" CTA on step 7.
  ...
  // Single source of truth for "load every wizard dataset". Mount AND
  // restart() both call this, so adding a future fetch is one edit and the
  // two paths cannot diverge. The mock adapter resolves synchronously today;
  // when HttpOnboardingApi (SP-3a) lands the `.catch` surfaces a real error
  // state instead of an unhandled rejection + blank screen.
  // Modeled wizard scaffolding (crawl tasks, media, questions, tones,
  // agents, deploy log). These resolve instantly even under
  // HttpOnboardingApi (no backend for them yet). Loaded once at mount so
  // step 0 is immediately interactive AND StepCrawl has its task list
  // ready to animate while the slow real analyze runs. Deliberately
  // decoupled from analyze so a ~25s extraction never blocks URL entry.
  const loadModeled = useCallback(async () => {
    try {
      // NOTE: media network, weekly questions, AND voice tones are NOT
      // loaded here. Under HttpOnboardingApi all three are REAL cortex-api
      // calls that need the brand profile (persisted by analyze) AND an
      // active brand session — neither exists at mount. They are fetched
      // in runAnalyze() after analyze succeeds, each in its OWN isolated
      // void-async block. The remaining datasets are instant local
      // constants under any adapter.
      ...
      // non-fatal — the wizard chrome still renders. The real error
      // surface belongs to the analyze call (loadStatus / error screen).
    }
  }, [api]);
  ...
  const runAnalyze = useCallback(async (analyzeUrl: string) => {
    try {
      ...
      // Weekly questions are likewise a REAL backend call. Separate
      // isolated block: a questions failure must not flip the analyze
      // status nor be coupled to the media/voice fetches.
      void (async () => {
        try {
          const lq = await api.getLiveQuestions();
          setLiveQuestions(lq);
        } catch {
          // Non-fatal: analyze succeeded; step 4 shows no questions rather
          // than crashing the wizard.
        }
      })();
      // Voice tones are likewise a REAL backend call (style-configured
      // against the brand profile analyze just persisted) — same d8345ff
      // shape as media/questions. Separate isolated block.
      void (async () => {
        try {
          const tones = await api.getVoiceTones();
          setVoiceTones(tones);
        } catch {
          // Non-fatal: analyze succeeded; step 5 falls back to constant voice samples.
        }
      })();
    } catch {
      setLoadStatus("error");
  ...
  // user-triggered at step 0 (onAnalyze) — NOT on mount — so we no longer
  // burn a real LLM extraction of the hardcoded INITIAL_URL before the
  // user has even typed their site.
  useEffect(() => {
  ...
  const restart = useCallback(() => {
    ...
    setLiveQuestions([]);
    // Voice tones are brand-specific and fetched per analyze (same as media/questions)
    // — clear so a re-analyze refetches for the new brand instead of showing
    // the previous brand's voice samples.
    setVoiceTones([]);
  ...
  const onAddBrand = useCallback(async () => {
    ...
    try {
      ...
      setAddBrandError(e instanceof Error ? e.message : "Couldn't create a new brand. Try again.");
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            One of the brand-analysis steps didn&apos;t respond. Nothing was
            lost — try again.
          </div>
          <OnbButton
            variant="primary"
  ...
  return (
    ...
    >
      {step <= 5 ? (
        ...
        // Step 7 — slimmer "Brand Agent online" topbar.
        <div
          style={{
            background: "#fff",
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 11, color: "var(--mly-ink-500)", marginTop: 1 }}>
                  {brand?.name ?? "Your brand"} · onboarding complete
                </div>
              </div>
            </div>
          ...
          <div className="onb-rail">
            {step === 0 ? (
              <StepWelcome
                ...
                setUrl={setUrl}
                onAnalyze={() => {
                  // Analyze the URL the user actually entered. Without this
                  // the wizard only ever shows the mount-time analysis of
                  // INITIAL_URL — invisible under the URL-ignoring mock,
                  // surfaced once HttpOnboardingApi (SP-3a) honors the arg.
                  setLoadStatus("loading");
                  void runAnalyze(url);
                  setStep(1);
                }}
          ...
          >
            ...
            >
              <span style={{ fontFamily: "var(--font-mono)" }}>
                Step {railStep + 1} / {RAIL_STEPS.length}
              </span>
              <span
                style={{
                  width: 4,
              ...
              <span style={{ color: "var(--mly-ink-400)" }}>
                Editable later from Brand settings
              </span>
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              ...
              {step < 5 ? (
                <OnbButton variant="primary" onClick={next} iconRight="arrow_forward">
                  Next: {RAIL_STEPS[railStep + 1]}
                </OnbButton>
              ) : null}
              {/* Step 5's launch CTA lives inside the page (amber hero), not the footer. */}
            </div>
          </div>
        </div>
      ) : null}
